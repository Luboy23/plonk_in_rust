use crate::{srs::Srs, Poly as MyPoly}; // 从当前 crate 中导入 `srs::Srs` 模块，并将 `Poly` 类型重命名为 `MyPoly`
use ark_bls12_381::{Bls12_381, Fr}; // 导入 `ark_bls12_381` 库中的 BLS12-381 配对曲线和有限域 `Fr`
use ark_ec::{AffineCurve, PairingEngine}; // 导入椭圆曲线库中的仿射曲线和配对引擎接口
use ark_poly::{univariate::DensePolynomial, Polynomial, UVPolynomial}; // 导入多项式库中的 `DensePolynomial` 和多项式接口
use std::{
    fmt::{Debug, Display}, // 从标准库导入调试和显示 trait
    ops::{Add, Mul, Neg, Sub}, // 从标准库导入四则运算 trait
};
pub mod srs;
...
impl<'a> KzgScheme<'a> {
    ...
    pub fn identify(&self) -> KzgCommitment {
