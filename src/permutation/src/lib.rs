...
pub struct Permutation<const C: usize> {
    perm: Vec<usize>, // 存储置换映射
}

#[derive(Debug)]
pub struct CompiledPermutation<const C: usize> {
    pub cols: [Vec<(Fr, Fr)>; C], // 存储 C 列，每列是 (标签, 值) 的元组
    pub cosets: [Fr; C], // 存储 cosets
    rows: usize, // 存储行数
}
...
impl<const C: usize> CompiledPermutation<C> {
    ...
    pub fn sigma_commitments(&self, scheme: &KzgScheme, domain: GeneralEvaluationDomain<Fr>) -> [KzgCommitment; C] {
        self.cols
            .iter()
            .map(|col| {
                let evals = col.iter().map(|cell| cell.1).collect();
